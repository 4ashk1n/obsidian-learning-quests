...
3. Build a topic map:
   - major concepts become regions of the same large quest map;
   - sections/subsections become quests;
...
All `requires` values must point to real quest IDs in the same JSON file.
...
- every quest has at least one task;
