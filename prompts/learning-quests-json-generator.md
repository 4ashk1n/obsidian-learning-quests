...
- Use `kind: "gate"` only for milestones, section entrances, joins, or major synthesis checkpoints.
...
- Give every gate an icon. Choose a clear icon for the region: analysis, algebra, geometry, probability, complex analysis, final synthesis, etc.
...
- icons use valid Iconify `prefix:name` identifiers when present;
